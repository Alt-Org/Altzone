...
public class KojuTrayPopulator : MonoBehaviour
{
    ...
    {
        var store = Storefront.Get(); //Access store data
        ...
        //Exits and debugs if fetching of items fails
        if (allFurniture == null || allFurniture.Count == 0)
        {
            Debug.LogWarning("⚠️ No furniture items found.");
            yield break;
        }

        //Populates the tray by instantiating a card and filling it with the data from store
