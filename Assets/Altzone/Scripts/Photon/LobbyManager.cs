...
namespace Altzone.Scripts.Lobby
{
    ...
    public class LobbyManager : MonoBehaviour, ILobbyCallbacks, IMatchmakingCallbacks, IOnEventCallback, IInRoomCallbacks, IConnectionCallbacks
    {
        ...
        private IEnumerator FormMatchFromQueue(string[] selected, int roomGameTypeInt, string clanName, int soulhomeRank)
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        TryGetQueueLocalTeammateUserId(localUserIdForQueuePair, out queueLocalTeammateUserId, out queueLocalTeammateUsername);
                    }
        ...
        private string GetPlayerName(string userId)
        {
            ...
            var player = PhotonRealtimeClient.LobbyCurrentRoom.Players.Values.FirstOrDefault(p => p.UserId == userId);
            if (player == null || string.IsNullOrEmpty(player.UserId)) return string.Empty;
            else return player.NickName;
        }
                        ...
                        if (premadeMode && PhotonRealtimeClient.InRoom && PhotonRealtimeClient.CurrentRoom != null)
                        {
                            ...
                            if (isQueueRoom)
                            {
                                try
                                {
                                    ...
                                    {
                                        _premadeTeammateUserId = premadeUserId2;
                                        _premadeTeammateUserName = premadeUsername2;
                                    }
                                    else if (localUserId == premadeUserId2)
                                    {
                                        _premadeTeammateUserId = premadeUserId1;
                                        _premadeTeammateUserName = premadeUsername1;
                                    }
        ...
        private bool TryGetQueueLocalTeammateUserId(string localUserId, out string teammateUserId, out string teammateUsername)
        {
            teammateUserId = string.Empty;
            teammateUsername = string.Empty;
            ...
            if (!string.IsNullOrEmpty(localLeaderId) && localLeaderId != localUserId)
            {
                if (playersById.ContainsKey(localLeaderId))
                {
                    ...
                    teammateUsername = playersById[localLeaderId].NickName;
                    return true;
                }
            ...
            if (followers.Count == 1)
            {
                ...
                teammateUsername = playersById[teammateUserId].NickName;
                return true;
            }
        ...
        private bool ShouldDeferTwoPlayerBlockEarlyStartForOneSidedPremadeExactSize(int requiredFollowers, int completeDuoCount, out string reason)
        {
            ...
            if (TryGetQueueLocalTeammateUserId(localUserId, out _, out _))
            {
                return false;
            }
                                ...
                                if (loopSelected.Count >= loopRequiredFollowers)
                                {
                                    ...
                                    int loopHumanCount = 0;
                                        if (twoPlayerBlockMode && loopCompleteDuoCount == 1)
                                        {
                                            ...
                                            try { localHasQueueTeammate = TryGetQueueLocalTeammateUserId(PhotonRealtimeClient.LocalPlayer?.UserId ?? string.Empty, out _, out _); } catch { }
        ...
        private IEnumerator LeaveAndAutoRequeue(GameType gameType)
        {
            try
            {
                ...
                if (PhotonRealtimeClient.LocalPlayer.IsMasterClient)
                {
                    ...
                    {
                        _isPremadeMatchmakingFlow = true;
                        if (localUserId == requeuePremadeUserId1)
                        {
                            _premadeTeammateUserId = requeuePremadeUserId2;
                            _premadeTeammateUserName = requeuePremadeUsername2;
                        }
                        else if (localUserId == requeuePremadeUserId2)
                        {
                            _premadeTeammateUserId = requeuePremadeUserId1;
                            _premadeTeammateUserName = requeuePremadeUsername1;
                        }
                    }
            ...
            try
            {
                ...
                if (CurrentRooms != null && CurrentRooms.Count > 0)
                {
                    ...
                    foreach (LobbyRoomInfo room in roomsList)
                    {
                        ...
                        int joinAttemptId = _joinAttemptTracker.BeginJoinAttempt(room.Name, GetTeammateIds());
                        try { PhotonRealtimeClient.JoinRoom(room.Name, GetTeammateIds()); } catch (Exception ex) { Debug.LogWarning($"JoinAttempt[{joinAttemptId}]: JoinRoom threw: {ex.Message}"); _joinAttemptTracker.MarkJoinAttemptFailure(joinAttemptId, -1, ex.Message); }
                        float joinStart = Time.time;
                        while (!PhotonRealtimeClient.InRoom && Time.time - joinStart < joinAttemptTimeout)
                        {
                            if (_joinAttemptTracker.IsAttemptCompleted(joinAttemptId)) break;
                            yield return null;
                        }
                        ...
                        {
                            if (_joinAttemptTracker.TryGetFailedJoinAttempt(joinAttemptId, out string failMsg))
                            {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(leaderRoomName))
                {
                    if (PhotonRealtimeClient.InLobby)
                    {
                        ...
                        try { PhotonRealtimeClient.JoinRoom(leaderRoomName); } catch (Exception ex) { Debug.LogWarning($"FollowLeaderToNewRoom: JoinAttempt[{joinAttemptId}] JoinRoom threw: {ex.Message}"); _joinAttemptTracker.MarkJoinAttemptFailure(joinAttemptId, -1, ex.Message); }
                        ...
                        {
                            if (_joinAttemptTracker.TryGetFailedJoinAttempt(joinAttemptId, out string failMsg)) Debug.LogWarning($"FollowLeaderToNewRoom: JoinAttempt[{joinAttemptId}] failed joining '{leaderRoomName}': {failMsg}");
                    ...
                    if (!newRoomJoined && queueRoomRequested && PhotonRealtimeClient.InLobby)
                    {
                        ...
                        while (!newRoomJoined && Time.time - queueRetryStart < 8f)
                        {
                                try
                                {
                                    int retryJoinAttemptId = _joinAttemptTracker.BeginJoinAttempt(leaderRoomName, followTeammates);
                ...
                if (attemptedFollowJoinCreate)
                {
                    ...
                    observedAttemptId = _joinAttemptTracker.CurrentJoinAttemptId;
                    Debug.Log($"FollowLeaderToNewRoom: awaiting JoinAttempt[{observedAttemptId}] result...");
                    float joinStart = Time.time;
                    while (!PhotonRealtimeClient.InRoom && Time.time - joinStart < 5f)
                    {
                        if (observedAttemptId != 0 && _joinAttemptTracker.IsAttemptCompleted(observedAttemptId)) break;
                        yield return null;
                    }
                    Debug.Log($"FollowLeaderToNewRoom: JoinAttempt[{observedAttemptId}] finished. InRoom={PhotonRealtimeClient.InRoom}");
                ...
                try
                {
                    ...
                    if (currentRoom2 != null && PhotonRealtimeClient.LocalPlayer != null && PhotonRealtimeClient.LocalPlayer.IsMasterClient)
                    {
                        ...
                        if (isQueueRoom2)
                        {
                            ...
                            currentRoom2.SetCustomProperty(PhotonBattleRoom.PremadeUsername1Key, string.Empty);
        ...
        public void OnJoinedRoom()
        {
            ...
            try
            {
                ...
                int matchedId = _joinAttemptTracker.FindJoinAttemptIdForRoomName(joinedRoomName);
                // If we found a matching attempt, mark success; otherwise mark current attempt if present.
                if (matchedId != 0) _joinAttemptTracker.MarkJoinAttemptSuccess(matchedId);
                else _joinAttemptTracker.MarkJoinAttemptSuccess(0);
            }
            catch { }
        ...
        {
            Debug.LogError($"JoinRoomFailed {returnCode} {message} (joinAttemptId={_joinAttemptTracker.LastIssuedAttemptId})");
