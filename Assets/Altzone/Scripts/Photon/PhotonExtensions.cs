...
public static class PhotonExtensions
{
    ...
    {
        RealtimeClient Client = new RealtimeClient(PhotonServerSettings.Global.AppSettings.Protocol);

        if (!Client.InRoom)
        {
            throw new UnityException($"Invalid connection state: {Client.State}");
        }
        if (room.PlayerCount > 0)
        {
            ...
            foreach (var otherPlayer in Client.CurrentRoom.Players)
                if (string.Equals(otherPlayer.Value.NickName, playerName, StringComparison.CurrentCultureIgnoreCase))
                {
                    // Assign new name to current player.
                    playerName = $"{playerName}{separator}{Client.LocalPlayer.ActorNumber}";
                    break;
                }
        }
        return playerName;
    ...
    private static void DoSetCustomProperty(this Player player, string key, object newValue, object currentValue)
    {
        ...
        var expectedProps = new PhotonHashtable { { key, currentValue } };
    ...
    private static string AsSorted(this PhotonHashtable dictionary)
    {
